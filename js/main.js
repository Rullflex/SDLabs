...
if ($(window).width() < 980) {
    $('.box__btn').css('display', 'none');
    $('.gallery__st-link').addClass('stretched-link');
    $('.gallery-block').css('position', 'static');
} else {
    $('.box__btn').css('display', 'inline-block');
    $('.gallery__st-link').removeClass('stretched-link'); 
    $('.gallery-block').css('position', 'relative'); 
}
if ($(window).width() < 768) {
    $('.box__btn').find('.btn__text').text('Смотреть фото');
} else {
    $('.box__btn').find('.btn__text').text('Смотреть фото проекта');
}
$(window).scroll(function() {
    let st = $(this).scrollTop();
    if (st > 400) {
        $('.menu__body').css({'width': '1030px', 'opacity' : '1'});
        $('.menu').css({
            'height' : '60px',
        ...
        $('.menu__body').css({'width' : '710px', 'opacity' : '1'});
        $('.menu').css({
            'height' : '91px',
        ...
        $('.menu__body').css({'width' : '710px', 'opacity' : '0'});
        $('.menu').css({
            'height' : '91px',
            'background-color' : 'rgba(255, 255, 255, 0)'
        });
    ...
    if ($('.sec3').hasClass('active')) {
        $('.menu__link-first').addClass('active');
    } else {
        $('.menu__link-first').removeClass('active');
    }
    if ($('.sec6').hasClass('active')) {
        $('.menu__link-second').addClass('active');
    } else {
        $('.menu__link-second').removeClass('active');
    }
    if ($('.sec8').hasClass('active')) {
        $('.menu__link-third').addClass('active');
    } else {
        $('.menu__link-third').removeClass('active');
    }
    if ($('.sec16').hasClass('active')) {
        $('.menu__link-four').addClass('active');
    } else {
        $('.menu__link-four').removeClass('active');
    }
...
if ($(window).width() > 1199) {
    $('.sec8__item').addClass('active');
} else {
    if ($('.sec8__item2').hasClass('active')) {
        $('.sec8__item1').removeClass('active');
        $('.tab__navitem2').addClass('active');
        $('.tab__navitem1').removeClass('active');
    } else {
        $('.sec8__item2').removeClass('active');
        $('.tab__navitem1').addClass('active');
        $('.tab__navitem2').removeClass('active');
    }
...
$(window).resize(function() {
    ...
    if ($(window).width() < 980) {
        ...
        $('.box__btn').css('display', 'inline-block');
        $('.gallery__st-link').removeClass('stretched-link'); 
        $('.gallery-block').css('position', 'absolute'); 
    }
    if ($(window).width() < 768) {
        $('.box__btn').find('.btn__text').text('Смотреть фото');
    } else {
...
$('.sec12__hidden-content').hide();
$('.sec12__spoiler').click(function(){
    if ($(window).width() >= 1200) {
        $('#sec12__toChange').css('height', '370px')
    }
    if ($(window).width() < 1200) {
        $('#sec12__toChange').css('height', '270px')
    }
    
    $('.sec12__hidden-content').show(250);
    $('.sec12__spoiler').hide();
});
var translation = 0;
$('.prev').click(function(){
    if (translation == 0) {
        $('.sec11__block-items').css('transform', 'translateX(-66.66%)');
        translation = -66.66;
    } else {
        translation += 33.33;
        $('.sec11__block-items').css('transform', 'translateX(' + translation + '%)');
    }
});
$('.next').click(function(){
    if (translation == -66.66) {
        $('.sec11__block-items').css('transform', 'translateX(0%)');
        translation = 0;
    } else {
        translation -= 33.33;
        $('.sec11__block-items').css('transform', 'translateX(' + translation + '%)');
...
//quiz
var quize_Mas = $('.quiz__content');
let progressbar_coef = 100 / (quize_Mas.length);
progressbar_coefnach = 0;
$('.progress-line').css('width', progressbar_coefnach + '%');
for (let i = 0, len = quize_Mas.length; i < len; i++) {
    $(quize_Mas[i]).attr("id", "quiz" + (i + 1));
    $(quize_Mas[i]).removeClass('quiz-active');
    if ($(quize_Mas[i]).attr('id') == 'quiz1') {
        $(quize_Mas[i]).show();
    } else {
        $(quize_Mas[i]).hide();
    }
...
$('.then').on('click', function() {
    ...
    var last = quize_id.toString().slice(-1);

    let title = $('#quiz' + last + ' .quiz__title h3').text();
    //alert(title);

    let otvet = $('#quiz' + last).find('.quiz-active .quiz-q').text();
    ...
    //массив Question - {вопрос, ответ}

    //console.log(Question)

    $('#quiz' + last).hide(300);
    let coef = Number(last);
    if ((Number(last)) == quize_Mas.length) {
        //форма 
        $('.quiz__form').show(300);
        $('.progress-line').css('width', progressbar_coef * coef + '%');
        if (coef < 2) {
            $('.progress-text').text('До получения предложения с точной стоимостью осталось ' + parseInt(6 - coef) + ' вопросов');
            $('.quiz__top-text').text(parseInt(coef + 1) + ' вопрос из 6');
        } else if (coef == 5){
            $('.progress-text').text('До получения предложения с точной стоимостью остался всего ' + parseInt(6 - coef) + ' вопрос');
            $('.quiz__top-text').text(parseInt(coef + 1) + ' вопрос из 6');
        } else if (coef == 6){
            $('.progress-text').text('Поздравляем! Вы получаете предложения с точной стоимостью');
            $('.quiz__top-text').text('Готово!');
            $('.quiz__buttons').css('display', 'none');
        } else {
            $('.progress-text').text('До получения предложения с точной стоимостью осталось ' + parseInt(6 - coef) + ' вопроса');
            $('.quiz__top-text').text(parseInt(coef + 1) + ' вопрос из 6');
        }
        if ((coef == 3) && $(window).width() > 768 && $(window).width() < 1200){
            $('.sec6__quiz').css('height', '940px');
        }
        if ((coef != 3) && $(window).width() > 768 && $(window).width() < 1200){
            $('.sec6__quiz').css('height', '753px');
        }
        if ((coef == 3) && $(window).width() < 768){
            $('.sec6__quiz').css('height', '453px');
        }
        if ((coef != 3) && $(window).width() < 768){
            $('.sec6__quiz').css('height', '353px');
        }
        if ((coef == 5) && $(window).width() < 768){
            $('.sec6__quiz').css('height', '453px');
        }
        if ((coef == 6) && $(window).width() < 768){
            $('.sec6__quiz').css('height', '453px');
        }
        $('.quiz-panel').hide();
        $('#sale-end').append($('.you-sale'));

        //печать скрытых полей ввода
        let text = "";
        for (let i = 0; i < Question.length; i++) {
            text += "<input type='hidden' name='" + del_spaces(Question[i]['title']) + "' value='" + Question[i]['otvet'] + "'>";
        }
        $('.getquest').append(text);
    } else {
        $('.progress-line').css('width', progressbar_coef * coef + '%');
        $('#quiz' + (Number(last) + 1)).show(300);
        if (coef < 2) {
            $('.progress-text').text('До получения предложения с точной стоимостью осталось ' + parseInt(6 - coef) + ' вопросов');
            $('.quiz__top-text').text(parseInt(coef + 1) + ' вопрос из 6');
        } else if (coef == 5){
        ...
        if ($('#quiz' + (Number(last) + 1)).attr('data-check') != 1) {
            $('.quiz__ovarlay').find('.then').prop("disabled", true);
        }
    }
...
$('.back').on('click', function() {
    ...
    $('#quiz' + last).hide(300);
    $('#quiz' + (Number(last) - 1)).show(300);
    $('#sale').text(Number(last - 2) * rub);
    // $('#quize' + (Number(last) - 1)).find('.then').prop("disabled", true);
    // $('#quize' + (Number(last) - 1)).find('.quize-buttons').hide();
})


$('.quiz__quest_item').on('click', function() {
    let id_quize = $(this).parent().parent().attr('id');
    //alert(id_quize);
    var q_mas = $('#' + id_quize).find('.quiz__quest_item');
    for (let i = 0, len = q_mas.length; i < len; i++) {
        $(q_mas[i]).removeClass('quiz-active');
        $(q_mas[i]).find('.circle-inner').removeClass('circle-active');
    }
    $(this).toggleClass('quiz-active');
    $(this).find('.circle-inner').toggleClass('circle-active');
    // $('.quize-buttons').show(300);
